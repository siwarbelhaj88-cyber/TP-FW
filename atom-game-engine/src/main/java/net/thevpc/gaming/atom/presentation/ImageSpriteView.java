...
public class ImageSpriteView extends DefaultSpriteView {
    ...
    public void draw(SpriteDrawingContext context) {
        ...
        int diameter = Math.min(rb.getWidth(), rb.getHeight());
        int x = rb.getX() + (rb.getWidth() - diameter) / 2;
        int y = rb.getY() + (rb.getHeight() - diameter) / 2;
        Shape circle = new java.awt.geom.Ellipse2D.Double(x, y, diameter, diameter);
        graphics.setClip(circle);
        graphics.drawImage(image, rb.getX(), rb.getY(), rb.getWidth(), rb.getHeight(), null);
